...
const ForgetPassword = ({ setPage }: any) => {
  ...
  const handleChange = (e: any) => {
    ...
    if (name === "phone") {
      setUserInfo((prevUserInfo) => ({
        ...
        errorMessage:
          !saudiPhoneNumberRegex.test(value) && value.length > 0
  ...
  const submitHandler = () => {
    ...
    if (!userInfo.isEmailShow) {
      setPage("number");
    }
    if (userInfo.phone === "" || userInfo.email === "") {
      setUserInfo((prevUserInfo) => ({
        ...prevUserInfo,
        emailOrPhoneError: "Please Enter Phone Or Email",
      }));
    } else {
      setUserInfo((prevUserInfo) => ({
        ...prevUserInfo,
      ...
      >
        <div className="w-full max-h-[574px] flex-col justify-start items-start gap-5 inline-flex">
          ...
          <div className="px-px py-[15px] w-full mb-3">
            ...
            {!(
              (!userInfo.isPhoneShow && userInfo.isEmailShow) ||
              (userInfo.isPhoneShow && !userInfo.isEmailShow)
            ) && (
