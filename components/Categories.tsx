...
const Categories = () => {
  ...
  const categories = categoryImage.map((val, i) => ({
    id: i + 1,
    image: val,
            ...
            {categories.map((val: any) => {
              return (
                ...
                >
                  <div
                    onClick={() =>
                      navigate.push(`/categories/${val.title.toLowerCase()}`)
                    }
                    className={`${
                      val.title === "Others" ? "bg-[#ffc700]" : "bg-white"
                    } p-6 rounded-2xl shadow-md flex flex-col items-center cursor-pointer transition-transform duration-300 hover:shadow-gray-600`}
                  >
                    <Image
                      className="w-[62px] h-[62px] object-cover mb-4 rounded-md"
                      src={val.image}
                      alt="Card Image"
                    />
