                    ...
                    <div className='w-full md:w-1/4 lg:w-1/4'>
                        ...
                        <div className="mb-4">
                            <div className="flex items-center cursor-pointer" onClick={toggleDropdown}>
                                <div className="text-black text-lg sm:text-xl font-bold ml-4">{name}</div>
                                <Image className={`object-cover ml-1 ${isDropdownOpen ? 'rotate-180 duration-1000' : 'duration-1000'}`} src={downArrow} alt="Card Image" />
                            </div>
                            {isDropdownOpen && (
                    ...
                    <div className='w-full md:w-3/4 lg:w-3/4 ml-0 md:ml-5 lg:ml-5 mt-1 sm:mt-16'>
                        <div className='sm:flex justify-center'>
                            <div className='w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 '>
                                <div className='text-black text-center font-bold p-1 bg-[#ffc700] rounded-md'>1000+ Auctions</div>
                            </div>
                            <div className='w-full flex justify-center items-end sm:flex-row sm:items-center sm:justify-end'>
                                <div className='mb-2 sm:mb-0'>
                                    <div className="hidden md:flex items-center justify-between">
                                        <div className="flex items-center space-x-2 py-3">
                                            ...
                                            <div><Image onClick={() => toggleView('list')} className={`${view === 'list' && 'bg-[#ffc700]'} rounded-full w-8 h-8 cursor-pointer p-1`} src={listView} alt="List View" /></div>
                                            <div><Image onClick={() => toggleView('grid')} className={`${view === 'grid' && 'bg-[#ffc700]'} rounded-full w-8 h-8 cursor-pointer p-1`} src={gridView} alt="Grid View" /></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-x-4 py-2">
                                    <div className="relative inline-block text-left">
                                        <div className='flex'>
                                            <div className="text-black-900 font-bold mr-2 mt-2 sm:mt-2">Sort by :</div>
                                            <div>
                                                <Select
                                                    value={sortByOptions.find(option => option.value === sortBy)}
                                                    onChange={(selectedOption: any) => handleSortByChange(selectedOption.value)}
                        ...
                        <div className={`grid ${view === 'list' ? 'grid-cols-1 gap-1' : 'sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4'} `}>
                            {Array.from({ length: 9 }).map((_, index) => (
                                <CategoryCard key={index} view={view} index={index} description={"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."} />
                            ))}
                        </div>

                        <div className='w-full text-md font-bold flex justify-center my-5'>
                            <button className="text-black text-md font-bold p-2 border border-black rounded-[5px] bg-white">Load More</button>
                        </div>
